...
var FlipShotButton=Class({visible:!0,create:function(a){this.parent=a;this.init();this.initEvents()},init:function(){var a=this.getImageUrl("previous_normal"),b=this.getImageUrl("next_normal"),c=this.getImageUrl("previous_normal_down"),d=this.getImageUrl("next_normal_down"),e=this.getImageUrl("slide_firstButton"),f=this.getImageUrl("slide_lastButton");this.leftBotton=$("<div aria-label='"+getLanguage("ariaPreviousPage","previous page")+"' tabindex='0' role='button' class='flip_button_left button'></div>");
this.leftBottonIcon=$("<img width='auto' height='auto' alt='leftBotton' src='"+a+"'/>");this.leftBotton.append(this.leftBottonIcon);this.rightBotton=$("<div aria-label='"+getLanguage("ariaNextPage","next page")+"' tabindex='0' role='button' class='flip_button_right button'></div>");this.rightBottonIcon=$("<img width='auto' height='auto' alt='rightBotton' src='"+b+"'/>");this.rightBotton.append(this.rightBottonIcon);this.firstButton=$("<div aria-label='"+getLanguage("ariaFirstPage","first page")+"' tabindex='0' role='button' class='flip_button_first button'><img width='40px' height='40px' alt='firstButton' src='"+
e+"'/>");this.lastButton=$("<div aria-label='"+getLanguage("ariaLastPage","last page")+"' tabindex='0' role='button' class='flip_button_last button'><img width='40px' height='40px' alt='lastButton' src='"+f+"'/></div>");var g=this;$("<img src='"+a+"'/>")[0].onload=function(){g.width=this.naturalWidth;g.height=this.naturalHeight;g.onResize();g.parent.append(g.leftBotton);g.parent.append(g.rightBotton);bookConfig.isFirstLastShortcutVisible&&(g.parent.append(g.firstButton),g.parent.append(g.lastButton))};
this.leftBottonIcon.asImageButton(c);this.rightBottonIcon.asImageButton(d);this.leftBotton.enableEnterClick();this.rightBotton.enableEnterClick();this.firstButton.enableEnterClick();this.lastButton.enableEnterClick()},getImageUrl:function(a){return window.toolbar_icons&&toolbar_icons[a]?"data:image/png;base64,"+toolbar_icons[a]:uiBaseURL+a+".png"},onResize:function(){this.left=toolBar.getLeftWidth();this.right=toolBar.getRightWidth();var a=BookInfo.top();1==BookInfo.getCurrentPageIndex()&&BookInfo.isDoublePage()&&
bookConfig.retainBookCenter&&bookConfig.FlipDirection&&"slide"!=bookConfig.FlipStyle.toLowerCase()&&(a+=BookInfo.height()/4);this.top=(BookInfo.height()-this.height)/2+a;this.minLeft=Math.max(this.left,10);this.minRight=Math.max(this.right,10);this.minBottom=windowHeight-a-BookInfo.height();a=$(".tableofcontent_form").outerWidth(!0);var b=this.minLeft+(global.frmTableOfContent&&global.frmTableOfContent.visible&&global.frmTableOfContent.nailUpStatus?a+10:0);this.leftBotton.css({left:this.left+(global.frmTableOfContent&&
global.frmTableOfContent.visible&&global.frmTableOfContent.nailUpStatus?a+10:0),top:this.top});this.firstButton.css({left:b,bottom:this.minBottom});this.rightBotton.css({right:this.right,top:this.top});this.lastButton.css({right:this.minRight,bottom:this.minBottom});this.showFirstOrLast()},animate:function(){var a=this.minLeft+(global.frmTableOfContent&&global.frmTableOfContent.visible&&global.frmTableOfContent.nailUpStatus?tocWidth+10:0);this.leftBotton.animate({left:this.left+(global.frmTableOfContent&&
global.frmTableOfContent.visible&&global.frmTableOfContent.nailUpStatus?tocWidth+10:0)},500);this.firstButton.animate({left:a},500);this.rightBotton.animate({right:this.right},500);this.lastButton.animate({right:this.minRight},500)},show:function(){this.leftBotton.show();this.firstButton.show();this.rightBotton.show();this.lastButton.show()},hide:function(){this.leftBotton.hide();this.firstButton.hide();this.rightBotton.hide();this.lastButton.hide()},initEvents:function(){this.leftBotton.bind(_event._click,
...
f,g;return $jscomp.asyncExecutePromiseGeneratorProgram(function(h){switch(h.nextAddress){case 1:if(0!=loadSearchFlag){b.doSearch(a);h.jumpTo(0);break}return isTouchDevice()?h.yield(getArticleImage(fixURLValue(bookConfig.searchTextJS)),5):(e=d=!1,f=function(){d&&e&&(loadSearchFlag=!0,this.doSearch(a))}.bind(b),h.yield(getArticleImage(fixURLValue(bookConfig.searchTextJS)),6));case 5:c=h.yieldResult;if(!c)return b.doSearch("",!0),h.return();loadJavascript(c,function(){loadSearchFlag=!0;this.doSearch(a)}.bind(b),
function(){loadSearchFlag=!0;this.doSearch(a)}.bind(b));h.jumpTo(0);break;case 6:c=h.yieldResult;if(!c)return b.doSearch("",!0),h.return();loadJavascript(c,function(){d=!0;f()},function(){d=!0;f()});g=String.format(bookConfig.searchPositionJS,BookInfo.getCurrentPageIndex());return h.yield(getArticleImage(fixURLValue(g)),7);case 7:return g=h.yieldResult,g?h.yield(loadAndFixPositionScript(BookInfo.getCurrentPageIndex(),g),8):(b.doSearch("",!0),h.return());case 8:e=!0,f(),h.jumpToEnd()}})},doSearch:function(a,
b){this.results=searchFromPages(textForPages,a);this.callback(this.results,b)},getList:function(){return this.results},clear:function(){this.results=[]}}),SearchItem=Class({create:function(a,b){this.fontColor=b.fontColor;this.parent=a;this.parm=b;this.isCrossBook=b.isCrossBook||!1;this.strategy=this.chooseStrategy(b);this.sDescription=this.strategy.getSearchResult();this.index=this.strategy.getSearchIndex();this.sPage=this.strategy.getPageOnSearch();this.sDescription?(this.initHtml(),this.initStyle(),
this.initEvent()):"function"==typeof b.emptyCallback&&b.emptyCallback()},chooseStrategy:function(a){return this.isCrossBook?new CrossBookStrategy(a):new SearchBookStrategy(a)},initHtml:function(){this.initElement();this.item.append(this.page);this.item.append(this.description);this.parent.append(this.item)},initElement:function(){this.item=$("<div class='item'></div>");rightToLeft?(this.page=$("<div class='page'>"+this.sPage+":"+getLanguage("lblPage","Page")+"</div>"),LanguageTool.isRightToLeft(this.parm.search_text)?
...
c&&c.empty().text(a);this.getPopupTipsVisible()&&(a=this.chatBotBtn.find("img")[0],a.complete?this.showChatBotBtnTips():a.addEventListener("load",function(){b.showChatBotBtnTips()}))},handleResize:function(){function a(l,n){function p(t){this.isResizing=!0;g=t.center.x;h=t.center.y;k=c.width();m=c.height();switch(n){case "right":c.css({right:"initial",left:t.center.x-k+"px"});break;case "left":c.css({right:"initial",left:t.center.x+"px"});break;case "top":c.css({bottom:"initial",top:t.center.y+"px"});
break;case "bottom":c.css({bottom:"initial",top:t.center.y-m+"px"})}$(l).css({cursor:"move"});d.css({display:"block"});b.frame.css({"box-shadow":"none",border:"none"})}function q(t){if(this.isResizing){var x=Math.min(t.center.x-g,e),v=Math.min(t.center.y-h,f);if(!(t.center.x>e||t.center.y>f||0>t.center.x||0>t.center.y))switch(n){case "right":c.css({width:k+x+"px"});break;case "left":c.css({left:t.center.x+"px",width:k-x+"px"});break;case "top":c.css({top:t.center.y+"px",height:m-v+"px"});break;case "bottom":c.css({height:m+
v+"px"})}}}function r(){this.isResizing=!1;"left"!=n&&"right"!=n||$(l).css({cursor:"ew-resize"});"top"!=n&&"bottom"!=n||$(l).css({cursor:"ns-resize"});d.css({display:"none"});b.frame.css({"box-shadow":"0 0 20px 0 rgba(0, 0, 0, 0.1)",border:"1px solid rgba(0,0,0,0.1)"})}var u=new Hammer(l);u.add(new Hammer.Pan({threshold:0,pointers:0}));u.on("panstart panmove panend",function(t){t.preventDefault();switch(t.type){case "panstart":p(t);break;case "panmove":q(t);break;case "panend":r(t)}})}document.querySelectorAll(".chatbot-resize-item").forEach(function(l){var n=
...
!global.chatbotPanel&&(global.chatbotPanel=new PhoneChatbot({parent:tmpContainer,formType:"chatbot",rightToLeft:rightToLeft,isPhone:!0}),(a=getHashParams("chatbotVisible"))&&"true"==a&&global.chatbotPanel.show())}else{bookConfig.BackgroundSoundButtonVisible&&("brief"!=global.templeteName||global.settingBody?bookConfig.BackgroundSoundURL&&!global.settingBody&&(global.settingBody=new PcSoundSettingForm({parent:tmpContainer,formType:"settingpc",rightToLeft:rightToLeft})):global.settingBody=new PcSoundSettingForm({parent:tmpContainer,
formType:"setting",rightToLeft:rightToLeft}));if(bookConfig.ThumbnailsButtonVisible&&!global.thumbnail||"popular"==global.templeteName&&!global.thumbnail)"catalog"==bookConfig.FlipStyle.toLowerCase()&&"lite"==bookConfig.ToolbarViewMode.toLowerCase()&&(global.thumbnail=new CatalogThumbnailBar(tmpContainer)),isSlideBook()&&"lite"==bookConfig.ToolbarViewMode.toLowerCase()&&(global.thumbnail=new SlideThumbnailBar(tmpContainer)),global.thumbnail=new ThumbnailForm({parent:tmpContainer,formType:"thumbnail",
rightToLeft:rightToLeft}),"thumbnails"==bookConfig.leftRightPnlShowOption&&(global.thumbnail.show(),global.thumbnail.stage&&global.thumbnail.stage.show());bookConfig.SearchButtonVisible&&!global.frmSearch&&(global.frmSearch=new PcSearchForm({parent:tmpContainer,formType:"search",rightToLeft:rightToLeft}));window.assistantConfig&&!global.assistant&&(global.assistant=new Assistant);if(0<=global.templeteName.indexOf("article")){if("article4"==global.templeteName){global.articleReader||(global.articleReader=
new PcArticleReader4({parent:tmpContainer,formType:"article",rightToLeft:rightToLeft}));return}global.articleReader||(global.articleReader=new PcArticleReader({parent:tmpContainer,formType:"article",rightToLeft:rightToLeft}));global.pcArticlePages||(global.pcArticlePages=new PcArticlePages({parent:tmpContainer,formType:"article",rightToLeft:rightToLeft,isPhone:!0}))}bookConfig.isChatBotButtonVisible&&!global.chatbotPanel&&(global.chatbotPanel=new ChatbotPanel(tmpContainer),(a=getHashParams("chatbotVisible"))&&
"true"==a&&global.chatbotPanel.show());bookConfig.isAccessibilityButtonVisible&&!global.accessiblePanel&&(global.accessiblePanel=new PcAccessibility($("body")),0<=window.location.hash.indexOf("accessibility")&&global.accessiblePanel.show())}bookConfig.QRCode&&!global.qrButton&&(global.qrButton=new QRButton(tmpContainer));bookConfig.appLargeLogoIcon&&!global.largeAppLogo&&(global.largeAppLogo=new AppLargeLogo(bookContainer));global.backgroundObj||(global.backgroundObj=new initBackground(tmpContainer));
global.auto_player||(global.auto_player=new AutoFlip);global.bgSound||(global.bgSound=new BackgroundSound);global.volumeControlBar||(global.volumeControlBar=new VolumeControlBar(tmpContainer));!isPad()&&!isPhone()&&"brief"!=global.templeteName||global.phoneWeChatShareHelp||(global.phoneWeChatShareHelp=new PhoneWeChatShareHelp($(document.body)));global.flipPageAudio||(global.flipPageAudio=new FlipByAudio(tmpContainer,global.flipByAudio));isPad()||isPhone()?bookConfig.EnableReportButton&&!global.reportPanel&&
(global.reportPanel=new PhoneReportPanel(tmpContainer)):bookConfig.EnableReportButton&&!global.reportPanel&&(global.reportPanel=new ReportPanel(tmpContainer));bookConfig.isAccessibilityButtonVisible&&!global.accessiblePanel&&(global.accessiblePanel=new PhoneAccessibility(tmpContainer));"1"==GetQueryString("showedit")&&!global.editItNowButton&&global.EditItNowButton&&(global.editItNowButton=new EditItNowButton(tmpContainer));isPhone()||isPad()||"Chinese"===UserGroups||!global.staticAd.adPosition||
global.staticSliderAd||(global.staticSliderAd=new StaticAd)}function setAccessibility(){$(tmpContainer).attr("aria-hidden","true");tmpContainer[0].querySelectorAll('input, button, a, textarea, [tabindex="0"]').forEach(function(b){b.setAttribute("tabindex","-1")});var a=tmpContainer[0].querySelector('input[aria-labelledby="currentPageIndex"]');a&&a.removeAttribute("aria-labelledby")}function initColorPanel(){global.colorPanel||(global.colorPanel=new JSColorPanel(tmpContainer))}
...
isPhone()||isPad()||(a?(global.toolBar&&global.toolBar.hide(!1),global.QRbutton&&global.QRbutton.hide(),bookType==BookType.catalog_book&&global.catalogBook.guidBar&&global.catalogBook.guidBar.hide(),global.flipShotButton&&flipShotButton.hide(),"popular"==global.templeteName&&(global.thumbnail&&global.thumbnail.lineStage&&global.thumbnail.lineStage.hide(),global.toolBar&&global.toolBar.showOrHideLogoBar&&global.toolBar.showOrHideLogoBar(!1)),MiniStyle.show()):(global.QRbutton&&global.QRbutton.show(),
("lite"!=bookConfig.ToolbarViewMode||bookType!=BookType.slide_book&&bookType!=BookType.singleSlide_book&&bookType!=BookType.catalog_book)&&toolBar&&toolBar.show(!1),bookType==BookType.catalog_book&&"lite"==bookConfig.ToolbarViewMode.toLowerCase()&&catalogBook.guidBar&&catalogBook.guidBar.show(),global.flipShotButton&&flipShotButton.show(),"popular"==global.templeteName&&(global.thumbnail&&global.thumbnail.lineStage&&global.thumbnail.lineStage.show(),global.toolBar&&global.toolBar.showOrHideLogoBar&&
global.toolBar.showOrHideLogoBar(!0)),MiniStyle.hide()))}}
